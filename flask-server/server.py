...
# Function to get proxy list from an external API
def get_proxies():
...
# Function to scrape product details from Flipkart using proxies
def scrape_flipkart_product(url, retries=3, delay=5):
    print(url)
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36",
    ...
    for attempt in range(retries):
        # Select a random proxy from the list, or use no proxy if the list is empty
        if proxies_list:
            ...
            print("No proxies available, making request without proxy.")
            proxies = None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Get the current timestamp
                time_scraped = datetime.now().strftime("%Y-%m-%d")
